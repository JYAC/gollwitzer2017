...
setwd("/Users/juliecachia/Desktop/RProjects/Replication Data")
...
gollwitzer2017 <- read_csv("test.csv")

#### Data exclusion / filtering

#Using filter function in tidyverse, exclude participants who failed attention check: 1) scoring below 6 on first item, and/or 2) not entering "Yes", "yes", or "Yes" into the text field of the second item

PassedACheck1 <- AttentionCheck1 > 5 #Finding which rows passed first attention check
PassedACheck2 <- grepl("yes", AttentionCheck2, ignore.case=TRUE) #Finding which rows passed second attention check and allowing all combination of cases for "yes"

gollwitzer2017_excl <- filter(gollwitzer2017, PassedACheck1 & PassedACheck2) #Filtering dataset to obtain only those rows that passed both attention checks

#### Prepare data for analysis - create columns etc.

#To Do: Name columns appropriately

#Create new columns
gollwitzer2017_excl %>%
  mutate(PatternDevAversion = mean(XXXXXX)) #Compute parttern deviancy aversion scores (mutate function, inserting mean of all 15 items)
  mutate(SocialDevAversion = mean(XXXXXX)) #Compute social deviancy aversion scores (mutate function, inserting mean of all 15 items)
  mutate(NeedForClosure = mean(XXXXXX)) #Compute need for closure scores (mutate function, inserting mean of all 15 items)
  
#Reverse code Item 1 of Neuroticism Scale:
  gollwitzer2017_excl[ , Neuroticism1] = 6 - gollwitzer2017_excl[ , Neuroticism1]
  
gollwitzer2017_excl %>%
  mutate(Neuroticism = mean(XXXXXX)) #Compute neuroticism scores (mutate function, inserting mean of 2 items)
```

### Confirmatory analysis
 
```{r eval=FALSE}
KeyAnalysis <- lm(SocialDevAversion ~ PatternDevAversion, data=gollwitzer2017_excl)  #build linear regression model
summary(KeyAnalysis)
```

*Side-by-side graph with original graph is ideal here* #No graph of this in original paper
```{r eval=FALSE}
ggplot(gollwitzer2017_excl, aes(x=PatternDevAversion, y=SocialDevAversion))+
  geom_point()+
...
```{r eval=FALSE}
AdditionalAnalysis <- lm(SocialDevAversion ~ PatternDevAversion + NeedForClosure + Neuroticism + PoliticalOrientation, data=gollwitzer2017_excl)  #build linear regression model with 3 covariates
summary(KeyAnalysis)
```
