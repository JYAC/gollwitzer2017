...
```{r eval=FALSE}
KeyAnalysis <- lm(SocialDevAversion ~ PatternDevAversion, data=gollwitzer2017)  #build linear regression model
summary(KeyAnalysis)
```

*Side-by-side graph with original graph is ideal here* #No graph of this in original paper
```{r eval=FALSE}
ggplot(gollwitzer2017, aes(x=PatternDevAversion, y=SocialDevAversion))+
  geom_point()+
...
```{r eval=FALSE}
AdditionalAnalysis <- lm(SocialDevAversion ~ PatternDevAversion + NeedForClosure + Neuroticism + PoliticalOrientation, data=gollwitzer2017)  #build linear regression model with 3 covariates
summary(KeyAnalysis)
```
